...
# Data analysis workflow examples {.tabset .tabset-pills}
...
knitr::opts_chunk$set(
    ...
    comment = "#>",
    warning = FALSE,
    message = FALSE, fig.width = 9, fig.height = 6)
```
...
se han eliminado `r nrow(proc_dt[[3]]) - nrow(feature_blank_na_rds)` y `r ncol(proc_dt[[3]]) - ncol(feature_blank_na_rds)` muestras
...
Para decidir un criterio para el CV límite, podemos utilizar la función `featureVarPlot()` para ver su distribución de valores en las features:
